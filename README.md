...
- 商品、低库存阈值、私有图片和预签名上传。
- 只通过独立库存页面和库存服务写入的“售出/新增”操作；展会售出支持最多 100 个商品、实际单价和展会结束日归属，订单和销售导入不改变库存。
- 金额必填、商品明细可选的订单，以及展会级最多 100 笔金额批量补录、确认、取消和退款流程。
- CSV、XLS、XLSX SumUp 导入向导：上传前必选展会；SumUp 作为支付方式，导入订单归入该展会。
- 财务销售额只读取订单/去重后的外部交易；库存归因金额单独提供商品、渠道、展会和价格分析，禁止双算。
- 财务趋势支持按 Tenant 时区的日/小时聚合；管理员可按 Tenant、系统用户、时间、渠道和展会查看跨租户订单与库存。
...
1. 选择归属展会后上传 CSV/XLS/XLSX；后端验证格式和大小、计算 SHA-256，并保存私有原文件。
2. 分析编码、分隔符、英/法表头、报告类型和数据粒度；上传和分析阶段不修改订单或库存。
3. 检查并修正列映射，预览标准化数据和错误。
4. 映射外部商品；匹配顺序为已保存映射、Reference/SKU、规范化名称、手动映射、未分配。
5. 查看新增、更新和错误，复核展会并确认可选商品映射；新订单写入 `sales_channel=EXHIBITION`、`payment_method=SUMUP`，导入不会改变库存。
6. 后端在行数上限保护的事务中执行幂等导入；完成后可查看错误 CSV，并可撤销满足条件的销售记录。
...
- 商品汇总与会计汇总只用于对账；任何库存变化都必须在库存页面单独操作。
