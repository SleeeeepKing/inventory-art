...
async function load() {
  loading.value = true
  try {
    const { data } = await api.get<PageResponse<ImportBatch> | ImportBatch[]>('/imports/sumup', {
...
async function downloadErrors(batch: ImportBatch) {
  try {
    const { data } = await api.get<Blob>(`/imports/sumup/${batch.id}/errors/export`, {
      responseType: 'blob',
    })
...
async function reverse(batch: ImportBatch) {
  try {
    await ElMessageBox.confirm(t('import.revertBody'), t('import.revertTitle'), {
      type: 'warning',
      confirmButtonText: t('import.revert'),
...
<template>
  <div class="page-stack">
    <PageHeader
      ...
      :title="t('import.listTitle')"
      :subtitle="t('import.subtitle')"
    >
      <template #actions
        ><ElButton type="primary" :icon="Plus" @click="router.push('/imports/new')">{{
          t('import.newImport')
        }}</ElButton></template
      >
    </PageHeader>
    <section class="panel data-panel">
      ...
      <ElTable
        ...
        row-key="id"
      >
        <ElTableColumn :label="t('import.file')" prop="originalFilename" min-width="230"
          ><template #default="scope"
            ><div class="file-cell">
              <span class="file-extension">{{
                (scope.row.originalFilename.split('.').pop() || '—').toUpperCase()
              }}</span
        ...
        >
        <ElTableColumn
          :label="t('import.salesEvent')"
          prop="eventName"
          min-width="220"
          show-overflow-tooltip
        />
        <ElTableColumn :label="t('import.type')" prop="importType" min-width="150" />
        <ElTableColumn :label="t('import.rows')" min-width="120" align="right"
          ><template #default="scope"
            >{{ number(scope.row.importedRows ?? scope.row.validRows ?? scope.row.totalRows) }} /
            {{ number(scope.row.totalRows) }}</template
          ></ElTableColumn
        >
        <ElTableColumn :label="t('common.status')" min-width="190"
          ><template #default="scope"><StatusPill :status="scope.row.status" /></template
        ></ElTableColumn>
        <ElTableColumn :label="t('import.started')" min-width="170"
          ><template #default="scope">{{ dateTime(scope.row.createdAt) }}</template></ElTableColumn
        >
        <ElTableColumn :label="t('common.actions')" width="210" fixed="right"
          ><template #default="scope"
            ><ElButton
              v-if="scope.row.errorRows"
              text
              :icon="Download"
              @click="downloadErrors(scope.row)"
              >{{ t('common.download') }}</ElButton
            ><ElButton
              v-if="['COMPLETED', 'COMPLETED_WITH_ERRORS'].includes(scope.row.status)"
              text
              type="danger"
              :icon="RefreshLeft"
              @click="reverse(scope.row)"
              >{{ t('import.revert') }}</ElButton
            ></template
          ></ElTableColumn
        >
      </ElTable>
      <EmptyState v-else :title="t('import.emptyTitle')" :body="t('import.emptyBody')"
        ><ElButton type="primary" :icon="Plus" @click="router.push('/imports/new')">{{
          t('import.newImport')
        }}</ElButton></EmptyState
      >
