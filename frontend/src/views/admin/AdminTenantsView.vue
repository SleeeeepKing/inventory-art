...
function openCreate() { editingId.value = undefined; clearErrors(); Object.assign(form, { name: '', slug: '', defaultCurrency: 'EUR', timezone: 'Europe/Paris', locale: 'fr-FR' }); dialogOpen.value = true }
function openEdit(tenant: AdminTenant) { editingId.value = tenant.id; clearErrors(); Object.assign(form, { name: tenant.name, slug: tenant.slug || '', defaultCurrency: tenant.defaultCurrency || tenant.currency || 'EUR', timezone: tenant.timezone, locale: tenant.locale }); dialogOpen.value = true }
async function save() {
  if (!validateForm()) { ElMessage.warning(t('errors.validation')); return }
  saving.value = true
...
<template>
  <div class="page-stack">
    ...
    <ElDialog v-model="dialogOpen" :title="t(editingId ? 'admin.editTenant' : 'admin.addTenant')" width="min(620px, 94vw)">
      <ElForm label-position="top" class="form-grid"><ElFormItem :label="t('admin.tenantName')" required :error="formErrors.name"><ElInput v-model="form.name" /></ElFormItem><ElFormItem :label="t('admin.slug')" required :error="formErrors.slug"><ElInput v-model="form.slug" :disabled="Boolean(editingId)" /><small v-if="!editingId" class="field-hint">{{ t('admin.slugHint') }}</small></ElFormItem><ElFormItem :label="t('admin.currency')"><ElSelect v-model="form.defaultCurrency"><ElOption label="EUR" value="EUR" /><ElOption label="USD" value="USD" /><ElOption label="GBP" value="GBP" /></ElSelect></ElFormItem><ElFormItem :label="t('admin.timezone')" :error="formErrors.timezone"><ElInput v-model="form.timezone" /></ElFormItem><ElFormItem :label="t('admin.businessLocale')"><ElSelect v-model="form.locale"><ElOption label="en" value="en" /><ElOption label="zh-CN" value="zh-CN" /><ElOption label="fr-FR" value="fr-FR" /></ElSelect></ElFormItem></ElForm>
