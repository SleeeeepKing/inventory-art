...
const chartOption = computed<EChartsCoreOption>(() => ({
  ...
  tooltip: {
    trigger: 'axis',
    valueFormatter: (value: unknown) => money(Number(value), data.value.currency),
  },
  xAxis: {
    type: 'category',
    data: data.value.dailySales.map((point) =>
      date(point.date, { month: 'short', day: 'numeric' }),
    ),
    boundaryGap: false,
  ...
  series: [
    {
      type: 'line',
      data: data.value.dailySales.map((point) => point.revenue),
      smooth: 0.32,
      symbolSize: 7,
      lineStyle: { width: 3, color: '#ec5b45' },
      itemStyle: { color: '#ec5b45', borderColor: '#fff', borderWidth: 2 },
      areaStyle: {
        color: {
          type: 'linear',
          x: 0,
          ...
          y2: 1,
          colorStops: [
            { offset: 0, color: 'rgba(236,91,69,.24)' },
            { offset: 1, color: 'rgba(236,91,69,0)' },
          ],
        },
      },
    },
  ...
  try {
    const [
      ...
      api.get<PageResponse<Product>>('/products', {
        params: { page: 0, size: 6, lowStock: true, enabled: true },
      }),
      api.get<PageResponse<Product>>('/products', {
        params: { page: 0, size: 100, enabled: true },
      }),
      api.get<PageResponse<Order>>('/orders', {
        params: { page: 0, size: 1, status: 'CONFIRMED' },
      }),
      api.get<PageResponse<Order>>('/orders', { params: { page: 0, size: 5 } }),
    ])
    const report = reportResponse.data
    const metrics =
      report.currencies.find((entry) => entry.currency === report.defaultCurrency) ||
      report.currencies[0]
    data.value = {
      ...
      unitsOnHand: normalizePage(productResponse.data).items.reduce(
        (sum, product) => sum + Number(product.currentStock || 0),
        0,
      ),
      dailySales: report.dailyTrend
        .filter((point) => point.currency === (metrics?.currency || report.defaultCurrency))
        .map((point) => ({ date: point.date, revenue: Number(point.netSales) })),
...
<template>
  <div v-loading="loading" class="page-stack">
    ...
    >
      <template #actions
        ><ElButton :icon="Refresh" @click="load">{{ t('common.refresh') }}</ElButton></template
      >
    </PageHeader>
    <section class="metric-grid">
      ...
      <MetricCard
        ...
        :note="t('dashboard.stockAttentionHint')"
        :tone="data.lowStockCount ? 'warning' : 'default'"
      />
    ...
    <section class="dashboard-grid">
      <article class="panel chart-panel">
        <div class="panel-heading">
          <div>
            <p class="eyebrow">{{ t('dashboard.lastSevenDays') }}</p>
      ...
      <article class="panel stock-panel">
        <div class="panel-heading">
          <div>
            <p class="eyebrow">{{ t('dashboard.lowStock') }}</p>
            <h2>{{ t('dashboard.stockAttention') }}</h2>
          </div>
          <ElButton text :icon="ArrowRight" @click="router.push('/products')">{{
            t('common.view')
          }}</ElButton>
        </div>
        <div v-if="data.lowStockProducts.length" class="stock-list">
          <div
            v-for="product in data.lowStockProducts.slice(0, 6)"
            :key="product.id"
            class="stock-row"
          >
            <span class="stock-row__mark"><Box /></span
            ><span
              ><strong>{{ product.name }}</strong
              ><small>{{ product.sku }}</small></span
            ><b>{{ number(product.currentStock) }}</b>
          </div>
        </div>
        <EmptyState
          v-else
          :title="t('dashboard.healthy')"
          :body="t('dashboard.stockAttentionHint')"
        />
      </article>
      <article class="panel recent-panel">
        <div class="panel-heading">
          <div>
            <p class="eyebrow">{{ t('orders.title') }}</p>
            <h2>{{ t('dashboard.recentOrders') }}</h2>
          </div>
          <ElButton text :icon="ArrowRight" @click="router.push('/orders')">{{
            t('common.view')
          }}</ElButton>
        </div>
        <ElTable
      ...
      <article class="panel quick-panel">
        <div class="panel-heading">
          <div>
            <p class="eyebrow">{{ t('common.actions') }}</p>
            <h2>{{ t('dashboard.quickActions') }}</h2>
          </div>
        </div>
        <button type="button" @click="router.push('/orders')">
          <DocumentAdd /><span>{{ t('dashboard.newOrder') }}</span
          ><ArrowRight />
        </button>
        ...
        <button type="button" @click="router.push('/imports/new')">
          <UploadFilled /><span>{{ t('dashboard.importSumUp') }}</span
          ><ArrowRight />
        </button>
      </article>
    </section>
