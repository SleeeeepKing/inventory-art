...
const localeOptions: Array<{ value: SupportedLocale; key: string }> = [
  { value: 'en', key: 'profile.english' },
  { value: 'zh-CN', key: 'profile.chinese' },
  { value: 'fr-FR', key: 'profile.french' },
]
...
<template>
  <div class="page-stack profile-page">
    ...
    <section class="settings-grid">
      <article class="panel settings-card">
        ...
        <ElForm label-position="top">
          <ElFormItem :label="t('profile.displayName')" required
            ><ElInput v-model="form.displayName" size="large"
          /></ElFormItem>
          <ElFormItem :label="t('profile.email')"
            ><ElInput :model-value="auth.user?.email" size="large" disabled
          /></ElFormItem>
        </ElForm>
      </article>
      <article class="panel settings-card language-card">
        ...
        <div class="locale-options">
          <label
            ...
            :key="option.value"
            :class="{ 'is-selected': form.preferredLocale === option.value }"
          >
            <input v-model="form.preferredLocale" type="radio" :value="option.value" />
            <span class="locale-code">{{ option.value }}</span
      ...
      <article class="panel settings-card tenant-card">
        <div class="settings-card__heading">
          ...
          <div>
            <h2>{{ t('profile.tenantSettings') }}</h2>
        ...
        <dl>
          <div>
            ...
            <dd>{{ auth.user?.tenant?.timezone || 'UTC' }}</dd>
          </div>
          <div>
            <dt>{{ t('profile.currency') }}</dt>
            <dd>{{ auth.user?.tenant?.defaultCurrency || 'EUR' }}</dd>
          </div>
          <div>
            <dt>{{ t('profile.businessLocale') }}</dt>
            <dd>{{ auth.user?.tenant?.locale || '—' }}</dd>
          </div>
        </dl>
      </article>
      <article class="panel settings-card">
        ...
        <ElForm label-position="top">
          <ElFormItem :label="t('profile.currentPassword')" required
            ><ElInput
          ...
          <ElFormItem :label="t('profile.confirmPassword')" required
            ><ElInput
              ...
              autocomplete="new-password"
          /></ElFormItem>
          <ElButton type="primary" :loading="passwordSaving" @click="changePassword">{{
    ...
    <div class="sticky-save">
      <ElButton type="primary" size="large" :loading="saving" @click="save">{{
