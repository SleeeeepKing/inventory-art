...
function assignFile(file?: File) {
  ...
  if (!['csv', 'xls', 'xlsx'].includes(extension || '')) {
    ElMessage.warning(t('validation.fileType'))
    return
  }
  if (file.size > 20 * 1024 * 1024) {
    ElMessage.warning(t('validation.fileSize'))
    return
  }
...
function formatPreviewRow(row: Record<string, unknown>) {
...
async function fetchProducts() {
  const { data } = await api.get<PageResponse<Product> | Product[]>('/products', {
    params: { page: 0, size: 100, enabled: true },
  })
...
async function fetchPreview() {
  ...
  for (const row of data.rows || []) {
    ...
    const sourceSku = String(
      normalized.productReference || normalized.productSku || normalized.sku || '',
    )
    ...
    if (existing) existing.occurrences = (existing.occurrences || 1) + 1
      ...
      matches.set(sourceKey, {
        sourceKey,
        sourceSku: sourceSku || undefined,
        sourceName: sourceName || sourceSku,
        occurrences: 1,
      })
  }
  preview.value = {
    ...
    impact: {
      orders: data.createsOrders ? data.estimatedNew : 0,
      transactions: data.estimatedNew,
      errors: data.errors,
    },
...
async function pollBatch(terminal: string[], onReady: () => Promise<void>) {
  ...
  if (data.status === 'FAILED') {
    busy.value = false
    ElMessage.error(t('status.FAILED'))
    return
  }
  pollTimer = setTimeout(() => {
    void pollBatch(terminal, onReady)
  }, 1000)
}

async function uploadAndAnalyze() {
  if (!selectedFile.value) {
    ElMessage.warning(t('import.noFile'))
    return
  }
  if (!eventId.value) {
    ElMessage.warning(t('import.eventRequired'))
    return
  }
  busy.value = true
  ...
  try {
    ...
    const { data } = await api.post<ImportBatch>('/imports/sumup/upload', body, {
    ...
    preview.value = {
      ...
      columns: analysis.sourceColumns.map((sourceColumn) => ({
        sourceColumn,
        suggestedField: analysis.suggestedMappings[sourceColumn] || '',
      })),
...
async function saveColumns() {
  ...
  try {
    const mappings = Object.fromEntries(
      Object.entries(columnMappings.value).filter(([, target]) => Boolean(target)),
    )
    await api.put(`/imports/sumup/${batch.value.id}/column-mapping`, {
  ...
  try {
    ...
    if (selected.length) {
      await api.put(`/imports/sumup/${batch.value.id}/product-mappings`, {
        expectedAnalysisVersion: preview.value.analysisVersion,
        mappings: selected.map((match) => ({
          externalProductReference: match.sourceSku || match.sourceKey,
          externalProductName: match.sourceName,
...
async function confirmImport() {
  ...
  busy.value = true
  try {
    await api.post(`/imports/sumup/${batch.value.id}/confirm`, {
      expectedAnalysisVersion: preview.value.analysisVersion,
    })
    await pollBatch(['COMPLETED', 'COMPLETED_WITH_ERRORS'], async () => {
      step.value = 4
      busy.value = false
    })
  } catch (error) {
...
async function downloadErrors() {
  if (!batch.value) return
  try {
    const { data } = await api.get<Blob>(`/imports/sumup/${batch.value.id}/errors/export`, {
      responseType: 'blob',
    })
    const url = URL.createObjectURL(data)
    const link = document.createElement('a')
    link.href = url
    link.download = `sumup-errors-${batch.value.id}.csv`
    link.click()
  ...
  <div class="page-stack import-wizard-page">
    ...
    <section class="panel wizard-panel">
      <ElSteps :active="step" finish-status="success" align-center>
        <ElStep :title="t('import.stepUpload')" /><ElStep :title="t('import.stepColumns')" /><ElStep
          :title="t('import.stepProducts')"
        /><ElStep :title="t('import.stepReview')" /><ElStep :title="t('import.stepResult')" />
      </ElSteps>

      <div v-if="step === 0" class="wizard-stage upload-stage">
        <ElForm label-position="top" class="import-event-context"
          ><ElFormItem :label="t('import.salesEvent')" required
            ><ElSelect
              v-model="eventId"
              filterable
              class="full-width"
              ...
              ><ElOption
                ...
                :value="event.id" /></ElSelect></ElFormItem
        ></ElForm>
        <input
          ref="fileInput"
          ...
          type="file"
          accept=".csv,.xls,.xlsx"
          @change="assignFile(($event.target as HTMLInputElement).files?.[0])"
        />
        <button
          class="file-drop"
          ...
          @drop.prevent="onDrop"
        >
          <UploadFilled /><strong>{{ t('import.dropTitle') }}</strong
          ><span>{{ t('import.dropBody') }}</span
          ><em>{{ t('import.chooseFile') }}</em>
        </button>
        <div v-if="selectedFile" class="selected-file">
          <Document /><span
            ><small>{{ t('import.selectedFile') }}</small
            ><strong>{{ selectedFile.name }}</strong></span
          ><b>{{ (selectedFile.size / 1024).toFixed(1) }} KB</b>
        </div>
        <ElProgress
          v-if="busy && uploadProgress"
          class="upload-progress"
          :percentage="uploadProgress"
          :stroke-width="8"
        />
        <div class="wizard-actions">
          <ElButton @click="router.push('/imports')">{{ t('common.cancel') }}</ElButton
          ><ElButton type="primary" :loading="busy" @click="uploadAndAnalyze">{{
            busy ? t('import.analyzing') : t('import.uploadAndAnalyze')
          }}</ElButton>
        </div>
      </div>
      ...
      <div v-else-if="step === 1" class="wizard-stage">
        <div class="stage-heading">
          <h2>{{ t('import.stepColumns') }}</h2>
          <p>{{ t('import.mappingHint') }}</p>
        </div>
        <ElTable :data="preview.columns || []">
          <ElTableColumn prop="sourceColumn" :label="t('import.sourceColumn')" min-width="180" />
          <ElTableColumn
            prop="sample"
            :label="t('import.sample')"
            min-width="220"
            show-overflow-tooltip
          />
          <ElTableColumn :label="t('import.targetField')" min-width="240"
            ><template #default="scope"
              ><ElSelect v-model="columnMappings[scope.row.sourceColumn]" class="full-width"
                ><ElOption
        ...
        <div class="wizard-actions">
          <ElButton @click="step = 0">{{ t('common.back') }}</ElButton
          ><ElButton type="primary" :loading="busy" @click="saveColumns">{{
            t('common.continue')
          }}</ElButton>
        </div>
      </div>
      ...
      <div v-else-if="step === 2" class="wizard-stage">
        <div class="stage-heading">
          <h2>{{ t('import.stepProducts') }}</h2>
          <p>{{ t('import.productMatchHint') }}</p>
        </div>
        <ElTable :data="productMatches">
          <ElTableColumn :label="t('import.sourceProduct')" min-width="240"
            ><template #default="scope"
              ><div class="cell-stack">
                <strong>{{ scope.row.sourceName }}</strong
                ><code class="sku-code">{{ scope.row.sourceSku }}</code>
              </div></template
            ></ElTableColumn
          >
          <ElTableColumn :label="t('import.rows')" prop="occurrences" width="100" align="right" />
          <ElTableColumn :label="t('import.matchedProduct')" min-width="280"
            ><template #default="scope"
              ><ElSelect
                ...
                filterable
                class="full-width"
                :placeholder="t('import.keepUnallocated')"
                ><ElOption
                  v-for="product in products"
                  :key="product.id"
                  :label="`${product.name} · ${product.sku}`"
                  :value="product.id" /></ElSelect></template
          ></ElTableColumn>
        </ElTable>
        <div class="wizard-actions">
          <ElButton @click="step = 1">{{ t('common.back') }}</ElButton
          ><ElButton type="primary" :loading="busy" @click="saveProducts">{{
            t('common.continue')
          }}</ElButton>
        </div>
      </div>
      ...
      <div v-else-if="step === 3" class="wizard-stage">
        <div class="stage-heading">
          <h2>{{ t('import.impactTitle') }}</h2>
          <p>{{ t('import.reviewWarning') }}</p>
        </div>
        <ElAlert
          :title="t('import.eventReview', { event: batch?.eventName || '—' })"
          type="info"
          show-icon
          :closable="false"
        />
        <div class="impact-grid">
          <div>
            <span>{{ t('import.impactOrders') }}</span
            ><strong>{{ number(impact.orders) }}</strong>
          </div>
          <div>
            <span>{{ t('import.impactTransactions') }}</span
            ><strong>{{ number(impact.transactions) }}</strong>
          </div>
          <div data-warning>
            <span>{{ t('import.impactErrors') }}</span
            ><strong>{{ number(impact.errors) }}</strong>
          </div>
        </div>
        <ElTable
          v-if="preview.sampleRows?.length"
          class="preview-table"
          :data="preview.sampleRows"
          max-height="360"
          ><ElTableColumn type="index" width="58" :label="t('import.row')" /><ElTableColumn
            :label="t('import.normalizedPreview')"
            ><template #default="scope">
              <pre class="preview-json">{{ formatPreviewRow(scope.row) }}</pre>
            </template></ElTableColumn
          ></ElTable
        >
        <ElAlert :title="t('import.reviewWarning')" type="warning" show-icon :closable="false" />
        <div class="wizard-actions">
          <ElButton @click="step = 2">{{ t('common.back') }}</ElButton
      ...
      <div v-else class="wizard-stage result-stage">
        <div class="result-mark"><Check /></div>
        <h2>{{ t('import.completedTitle') }}</h2>
        <p>{{ t('import.completedBody', { count: batch?.importedRows || 0 }) }}</p>
        <StatusPill v-if="batch" :status="batch.status" />
        <div class="impact-grid">
          <div>
            <span>{{ t('import.importedRows') }}</span
            ><strong>{{ number(batch?.importedRows) }}</strong>
          </div>
          <div>
            <span>{{ t('import.updatedRows') }}</span
            ><strong>{{ number(batch?.updatedRows) }}</strong>
          </div>
          <div>
            <span>{{ t('import.duplicateRows') }}</span
            ><strong>{{ number(batch?.duplicateRows) }}</strong>
          </div>
          <div>
            <span>{{ t('import.skippedRows') }}</span
            ><strong>{{ number(batch?.skippedRows) }}</strong>
          </div>
          <div data-warning>
            <span>{{ t('import.impactErrors') }}</span
            ><strong>{{ number(batch?.errorRows) }}</strong>
          </div>
        </div>
        <div class="wizard-actions wizard-actions--center">
          <ElButton v-if="batch?.errorRows" :icon="Download" @click="downloadErrors">{{
            t('import.downloadErrors')
          }}</ElButton
          ><ElButton type="primary" @click="router.push('/imports')">{{
            t('import.viewImports')
          }}</ElButton>
        </div>
      </div>
    </section>
