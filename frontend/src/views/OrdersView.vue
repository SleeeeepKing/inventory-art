...
const batchConfirmOrders = computed(() =>
  selectedOrders.value.filter((order) => order.status === 'DRAFT'),
)
const batchCancelOrders = computed(() =>
  selectedOrders.value.filter((order) => ['DRAFT', 'CONFIRMED'].includes(order.status)),
)
...
function changeChannel(channel: string) {
  if (channel !== 'EXHIBITION') form.eventId = ''
}
function selectEvent() {
  if (form.eventId) form.salesChannel = 'EXHIBITION'
}
...
async function batchTransition(action: 'confirm' | 'cancel') {
  ...
  try {
    await ElMessageBox.confirm(
      t(action === 'confirm' ? 'orders.batchConfirmBody' : 'orders.batchCancelBody', {
        count: candidates.length,
      }),
      t(action === 'confirm' ? 'orders.batchConfirmTitle' : 'orders.batchCancelTitle'),
      {
        type: 'warning',
        confirmButtonText: t(action === 'confirm' ? 'orders.batchConfirm' : 'orders.batchCancel'),
        cancelButtonText: t('common.cancel'),
      },
    )
    batchProcessing.value = true
    const { data } = await api.post<OrderBatchResponse>(`/orders/batch-${action}`, {
    ...
    if (data.failed.length) {
      ...
      ElMessage.warning(
        t('orders.batchPartial', { succeeded: data.succeeded.length, failed: data.failed.length }),
      )
    } else {
...
async function transition(order: Order, action: 'confirm' | 'cancel' | 'refund') {
  try {
    await ElMessageBox.confirm(t(`orders.${action}Body`), t(`orders.${action}Title`), {
      type: 'warning',
      confirmButtonText: t(`orders.${action}Order`),
      cancelButtonText: t('common.cancel'),
    })
    if (action === 'refund') {
    ...
    ElMessage.success(
      t(
        `orders.${action === 'confirm' ? 'confirmed' : action === 'cancel' ? 'cancelled' : 'refunded'}`,
      ),
    )
    await load()
  } catch (error) {
  ...
  <div class="page-stack">
    ...
    <section class="panel data-panel">
      <div class="table-toolbar">
        ...
        >
          <ElOption :label="t('orders.filters.draft')" value="DRAFT" /><ElOption
            :label="t('orders.filters.active')"
            value="CONFIRMED"
          /><ElOption :label="t('orders.filters.completed')" value="COMPLETED" /><ElOption
      ...
      <div v-if="selectedOrders.length" class="table-toolbar">
        ...
        <ElButton
          ...
          :disabled="!batchConfirmOrders.length"
          @click="batchTransition('confirm')"
          >{{ t('orders.batchConfirm') }} ({{ batchConfirmOrders.length }})</ElButton
        >
        <ElButton
          type="danger"
          plain
          :loading="batchProcessing"
          :disabled="!batchCancelOrders.length"
          @click="batchTransition('cancel')"
          >{{ t('orders.batchCancel') }} ({{ batchCancelOrders.length }})</ElButton
        >
      </div>
      <ElTable
        ...
        @expand-change="loadOrderDetails"
      >
        <ElTableColumn type="selection" width="48" :selectable="canBatchSelect" />
        <ElTableColumn type="expand"
          ><template #default="scope"
            ><div class="order-lines">
              <div v-for="item in scope.row.items" :key="item.id || item.productId">
                <span
                  ><strong>{{ item.productName || item.sku }}</strong
                  ><small>{{ item.sku }}</small></span
                ><span>{{ t('orders.quantity') }}: {{ item.quantity }}</span
                ><b>{{
                  money(item.lineTotal ?? item.quantity * item.unitPrice, scope.row.currency)
                }}</b>
              </div>
              <p v-if="!scope.row.items?.length">{{ t('orders.noItems') }}</p>
            </div></template
          ></ElTableColumn
        >
        <ElTableColumn prop="orderNumber" :label="t('orders.orderNumber')" min-width="150"
          ><template #default="scope"
            ><code class="order-code">{{ scope.row.orderNumber }}</code></template
          ></ElTableColumn
        >
        <ElTableColumn :label="t('orders.customer')" min-width="180"
          ><template #default="scope"
            ><div class="cell-stack">
              <strong>{{ scope.row.customerName || '—' }}</strong
              ><small>{{ scope.row.customerEmail }}</small>
            </div></template
          ></ElTableColumn
        >
        <ElTableColumn :label="t('orders.orderedAt')" min-width="165"
          ><template #default="scope">{{ dateTime(scope.row.orderDate) }}</template></ElTableColumn
        >
        <ElTableColumn :label="t('orders.event')" min-width="220"
          ><template #default="scope">{{ scope.row.eventName || '—' }}</template></ElTableColumn
        >
        <ElTableColumn :label="t('orders.source')" prop="source" min-width="110" />
        <ElTableColumn :label="t('common.status')" min-width="170"
          ><template #default="scope"><StatusPill :status="scope.row.status" /></template
        ></ElTableColumn>
        <ElTableColumn :label="t('orders.total')" min-width="130" align="right"
          ><template #default="scope"
            ><strong>{{ money(scope.row.totalAmount, scope.row.currency) }}</strong></template
          ></ElTableColumn
        >
        <ElTableColumn :label="t('common.actions')" width="175" fixed="right"
          ><template #default="scope"
            ><ElTooltip v-if="scope.row.status === 'DRAFT'" :content="t('orders.confirmOrder')"
              ><ElButton
                circle
                size="small"
                type="primary"
                :icon="Check"
                @click="transition(scope.row, 'confirm')" /></ElTooltip
            ><ElTooltip v-if="scope.row.status === 'CONFIRMED'" :content="t('orders.cancelOrder')"
              ><ElButton
                circle
                size="small"
                type="danger"
                plain
                :icon="Close"
                @click="transition(scope.row, 'cancel')" /></ElTooltip
            ><ElButton
              v-if="['CONFIRMED', 'COMPLETED', 'PARTIALLY_REFUNDED'].includes(scope.row.status)"
              text
              size="small"
              @click="transition(scope.row, 'refund')"
              >{{ t('orders.refundOrder') }}</ElButton
            ></template
          ></ElTableColumn
        >
      </ElTable>
    ...
    >
      <ElForm label-position="top" class="form-grid">
        <ElFormItem :label="t('orders.customerName')"
          ><ElInput v-model="form.customerName"
        /></ElFormItem>
        <ElFormItem :label="t('orders.customerEmail')"
          ><ElInput v-model="form.customerEmail" type="email"
        /></ElFormItem>
        <ElFormItem :label="t('orders.orderedAt')"
          ><ElDatePicker
            ...
            class="full-width"
        /></ElFormItem>
        <ElFormItem :label="t('orders.channel')"
          ><ElSelect v-model="form.salesChannel" class="full-width" @change="changeChannel"
        ...
        <div class="span-2 order-editor">
          <div class="order-editor__heading">
            <span
              ...
              ><small>{{ t('orders.itemsOptional') }}</small></span
            ><ElButton text :icon="Plus" @click="addItem">{{ t('orders.addItem') }}</ElButton>
          </div>
          ...
          <div class="order-total">
            ...
            ><strong>{{ money(form.totalAmount, form.currency) }}</strong>
          </div>
        </div>
        <ElFormItem class="span-2" :label="t('orders.notes')"
          ><ElInput v-model="form.customerNote" type="textarea" :rows="3"
        /></ElFormItem>
      </ElForm>
      <template #footer
        ><ElButton @click="dialogOpen = false">{{ t('common.cancel') }}</ElButton
        ><ElButton type="primary" :loading="saving" @click="createOrder">{{
      ...
      <ElForm label-position="top" class="form-grid batch-order-context">
        ...
        <ElFormItem :label="t('reports.paymentMethods')" required
          ><ElSelect v-model="batchForm.paymentMethod" class="full-width"
            ><ElOption :label="t('reports.labels.CASH')" value="CASH" /><ElOption
              :label="t('reports.labels.CARD')"
              value="CARD" /><ElOption label="SumUp" value="SUMUP" /><ElOption
              :label="t('reports.labels.OTHER')"
              value="OTHER" /></ElSelect
        ></ElFormItem>
      </ElForm>
      <div class="batch-amount-editor">
        <div class="order-editor__heading">
          <span
            ...
            ><small>{{ t('common.items', { count: batchForm.amounts.length }) }}</small></span
          ><ElButton
            text
            :icon="Plus"
    ...
    <ElDialog v-model="batchResultOpen" :title="t('orders.batchFailures')" width="min(620px, 94vw)">
      <ElTable :data="batchFailures">
        <ElTableColumn :label="t('orders.orderNumber')" min-width="180"
          ><template #default="scope">{{
            scope.row.orderNumber || scope.row.id
