...
async function load() {
  ...
  try {
    ...
    const { data } = await api.get<BackendDashboard>(endpoint, { params: { start: formatQueryDate(range.value[0]), end: formatQueryDate(range.value[1]), tenantId: auth.isAdmin && selectedTenantId.value ? selectedTenantId.value : undefined } })
