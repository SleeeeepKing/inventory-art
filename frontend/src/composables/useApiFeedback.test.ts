...
vi.mock('vue-i18n', () => ({
  useI18n: () => ({
    t: (key: string, params?: Record<string, string>) => params?.traceId ? `${key}:${params.traceId}` : key,
  }),
}))
...
describe('API error feedback', () => {
  ...
  it('localizes known error codes', () => {
    ...
    const fields = showError(apiError(400, { code: 'INSUFFICIENT_STOCK', message: 'internal message' }))
  ...
  it('shows and returns field-level validation details', () => {
    const { showError } = useApiFeedback()
    const fields = showError(apiError(400, { code: 'VALIDATION_ERROR', fieldErrors: { quantity: 'must be positive' } }))
  ...
  it('shows a specific backend message for an unmapped client error', () => {
    const { showError } = useApiFeedback()
    showError(apiError(422, { code: 'NEW_BUSINESS_RULE', message: 'This order cannot be processed today' }))
  ...
  it('hides server details and includes the trace id for server errors', () => {
    const { showError } = useApiFeedback()
    showError(apiError(500, { code: 'INTERNAL_ERROR', message: 'database password leaked', traceId: 'trace-1' }))
