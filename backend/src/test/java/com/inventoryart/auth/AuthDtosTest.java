...
class AuthDtosTest {
    ...
    void userResponseIncludesAccountStateAndBusinessFormatting() {
        Tenant tenant = new Tenant(UUID.randomUUID(), "Studio", "studio", "EUR", "Europe/Paris", "fr-FR");
        User user = new User(UUID.randomUUID(), tenant.getId(), "artist", "artist@example.com", "hash", "Artist", UserRole.USER);
        ...
        assertThat(response.tenant()).isEqualTo(new AuthDtos.TenantSummary(
            tenant.getId(), "Studio", "EUR", "Europe/Paris", "fr-FR"));
    }

    @Test
    void globalAdministratorHasNoTenantSummary() {
        User admin = new User(UUID.randomUUID(), null, "admin", "admin@example.com", "hash", "Admin", UserRole.ADMIN);
