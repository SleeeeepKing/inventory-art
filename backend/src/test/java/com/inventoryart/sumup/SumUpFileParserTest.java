...
class SumUpFileParserTest {
    ...
    void detectsFrenchSemicolonHeaderAfterPreambleAndWindows1252() throws Exception {
        String csv = "Rapport des transactions SumUp\r\nCréé le 11/07/2026\r\n"
        ...
        SumUpFileParser.ParseMetadata metadata = parser.parse(new ByteArrayInputStream(
            csv.getBytes(Charset.forName("windows-1252"))), "transactions.csv", (number, row) -> rows.add(row));
    ...
    void parsesXlsxWithSaxAndSkipsTitleRows() throws Exception {
        ...
        SumUpFileParser.ParseMetadata metadata = parser.parse(new ByteArrayInputStream(workbook), "orders.xlsx",
            (number, row) -> rows.add(row));

        assertThat(metadata.encoding()).isEqualTo("OOXML-SAX");
        assertThat(metadata.headers()).containsExactly("Transaction ID", "Product", "Reference", "Quantity", "Revenue");
        assertThat(rows).singleElement().satisfies(row -> assertThat(row).containsEntry("Reference", "ART-1"));
    }
    ...
    void stillSupportsBoundedLegacyXlsParsing() throws Exception {
        ...
        SumUpFileParser.ParseMetadata metadata = parser.parse(new ByteArrayInputStream(workbook), "orders.xls",
            (number, row) -> rows.add(row));
    ...
    void normalizesAmountsAndMasksPanInUnexpectedColumn() {
        Map<String, Object> sanitized = SumUpNormalizer.sanitize(Map.of(
            "Comment", "customer 4111 1111 1111 1111", "Montant", "1 234,56", "Devise", "eur",
            "Date", "10/07/2026"));
        Map<String, Object> normalized = SumUpNormalizer.normalize(sanitized, Map.of(), ZoneId.of("Europe/Paris"));
        ...
        assertThat(SumUpNormalizer.fingerprint(UUID.fromString("00000000-0000-0000-0000-000000000001"), normalized))
            .hasSize(64);
    }

    @Test
    void canonicalizesFrenchTransactionEnums() {
        Map<String, Object> normalized = SumUpNormalizer.normalize(Map.of(
            "Statut", "Réussi", "Type", "Paiement", "Mode de paiement", "Carte"), Map.of(), ZoneId.of("UTC"));
        assertThat(normalized).containsEntry("status", "SUCCESSFUL")
            .containsEntry("type", "PAYMENT").containsEntry("paymentMethod", "CARD");
    }
    ...
    private static byte[] workbook(Workbook workbook) throws Exception {
        try (workbook; ByteArrayOutputStream output = new ByteArrayOutputStream()) {
            ...
            List<String> headers = List.of("Transaction ID", "Product", "Reference", "Quantity", "Revenue");
