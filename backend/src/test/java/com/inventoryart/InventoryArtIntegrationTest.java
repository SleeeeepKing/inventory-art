...
class InventoryArtIntegrationTest {
  ...
  void loginRefreshAndLocalePersistence() throws Exception {
    ...
    String body =
        json.writeValueAsString(
            Map.of("username", f.user.getUsername(), "password", "ValidPassword123!"));
    var login =
        mvc.perform(
                post("/api/v1/auth/login").contentType(MediaType.APPLICATION_JSON).content(body))
            .andExpect(status().isOk())
            .andExpect(jsonPath("$.accessToken").isString())
            .andExpect(jsonPath("$.user.preferredLocale").value("en"))
            .andReturn();
    var oldCookie = Objects.requireNonNull(login.getResponse().getCookie("refresh_token"));
    var refreshed =
        mvc.perform(post("/api/v1/auth/refresh").cookie(oldCookie))
            .andExpect(status().isOk())
            .andExpect(jsonPath("$.accessToken").isString())
            .andReturn();
    var replacementCookie =
        Objects.requireNonNull(refreshed.getResponse().getCookie("refresh_token"));
    mvc.perform(post("/api/v1/auth/refresh").cookie(oldCookie))
        .andExpect(status().isUnauthorized())
        .andExpect(jsonPath("$.code").value("INVALID_REFRESH_TOKEN"));
    mvc.perform(post("/api/v1/auth/refresh").cookie(replacementCookie))
        .andExpect(status().isUnauthorized())
        .andExpect(jsonPath("$.code").value("INVALID_REFRESH_TOKEN"));
    mvc.perform(
            patch("/api/v1/profile")
                ...
                .content("{\"displayName\":\"Nom Français\",\"preferredLocale\":\"fr-FR\"}"))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.preferredLocale").value("fr-FR"));
    assertThat(users.findById(f.user.getId()).orElseThrow().getPreferredLocale())
        .isEqualTo("fr-FR");
  }

  @Test
  void localeValidationIsolationAndAdminDefaultAreEnforced() throws Exception {
    ...
    User admin =
        users.save(
            new User(
                UUID.randomUUID(),
                null,
                "locale-admin-" + UUID.randomUUID(),
                "locale-admin-" + UUID.randomUUID() + "@test.local",
  ...
  @Test
  void usersChangeTheirOwnPasswordAndAdminsResetAnyPassword() throws Exception {
    Fixture f = fixture("password");
    String loginBody =
        json.writeValueAsString(
            Map.of("username", f.user.getUsername(), "password", "ValidPassword123!"));
    var login =
    ...
    mvc.perform(
            post("/api/v1/profile/password")
                ...
                .contentType(MediaType.APPLICATION_JSON)
                .content(
                    "{\"currentPassword\":\"wrong-password\",\"newPassword\":\"ChangedPassword123!\"}"))
        .andExpect(status().isBadRequest())
    ...
    mvc.perform(
            post("/api/v1/profile/password")
                ...
                .contentType(MediaType.APPLICATION_JSON)
                .content(
                    "{\"currentPassword\":\"ValidPassword123!\",\"newPassword\":\"ChangedPassword123!\"}"))
    ...
    String changedLogin =
        json.writeValueAsString(
            Map.of("username", f.user.getUsername(), "password", "ChangedPassword123!"));
    var changedSession =
        mvc.perform(
                post("/api/v1/auth/login")
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(changedLogin))
            .andExpect(status().isOk())
            .andReturn();
    var changedCookie =
        Objects.requireNonNull(changedSession.getResponse().getCookie("refresh_token"));
    ...
    User admin =
        users.save(
            new User(
                UUID.randomUUID(),
                null,
                "password-admin-" + UUID.randomUUID(),
                "password-admin-" + UUID.randomUUID() + "@test.local",
                passwords.encode("ValidPassword123!"),
                "Admin",
                UserRole.ADMIN));
    mvc.perform(
            post("/api/v1/admin/users/{id}/reset-password", f.user.getId())
                .with(adminJwt(admin))
                .contentType(MediaType.APPLICATION_JSON)
    ...
    String resetLogin =
        json.writeValueAsString(
            Map.of("username", f.user.getUsername(), "password", "AdminResetPassword123!"));
    mvc.perform(
            post("/api/v1/auth/login").contentType(MediaType.APPLICATION_JSON).content(resetLogin))
        .andExpect(status().isOk());
    assertThat(audits.findAll())
        .anyMatch(
            a ->
                a.getAction().equals("PASSWORD_CHANGE")
                    && a.getResourceId().equals(f.user.getId()));
    assertThat(audits.findAll())
        .anyMatch(
            a ->
                a.getAction().equals("USER_PASSWORD_RESET")
                    && a.getResourceId().equals(f.user.getId()));
  }

  @Test
  void tenantResourcesAreNotDiscoverableAcrossUsers() throws Exception {
    Fixture a = fixture("tenant-a"), b = fixture("tenant-b");
    Product other = product(b, "OTHER-1", 5);
    mvc.perform(get("/api/v1/products/{id}", other.getId()).with(userJwt(a)))
    ...
    mvc.perform(
        ...
        .andExpect(status().isNotFound());
    mvc.perform(
            put("/api/v1/inventory/stock/{id}", other.getId())
                .with(userJwt(a))
                .contentType(MediaType.APPLICATION_JSON)
                .content("{\"quantity\":2}"))
        .andExpect(status().isNotFound());
  ...
  @Test
  void salesEventsAreTenantScopedFilterOrdersAndRejectDisabledEvents() throws Exception {
    Fixture first = fixture("event-first"), second = fixture("event-second");
    Product product = product(first, "EVENT", 5);
    String created =
        mvc.perform(
                post("/api/v1/sales-events")
                    ...
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(
                        "{\"name\":\"JAPAN EXPO PARIS 2026\",\"startDate\":\"2026-07-01\",\"endDate\":\"2026-07-05\"}"))
            .andExpect(status().isCreated())
            .andExpect(jsonPath("$.name").value("JAPAN EXPO PARIS 2026"))
            .andReturn()
            .getResponse()
            .getContentAsString();
    ...
    String disposable =
        mvc.perform(
                post("/api/v1/sales-events")
                    ...
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(
                        "{\"name\":\"One-off fair\",\"startDate\":\"2026-06-01\",\"endDate\":\"2026-06-01\"}"))
            .andExpect(status().isCreated())
            .andReturn()
            .getResponse()
            .getContentAsString();
    ...
    mvc.perform(get("/api/v1/sales-events/{id}", disposableId).with(userJwt(first)))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.name").value("One-off fair"));
    ...
    mvc.perform(
            put("/api/v1/sales-events/{id}", eventId)
                .with(userJwt(second))
                .contentType(MediaType.APPLICATION_JSON)
                .content(
                    "{\"name\":\"Changed\",\"startDate\":\"2026-07-01\",\"endDate\":\"2026-07-05\",\"enabled\":true}"))
        .andExpect(status().isNotFound());
    ...
    mvc.perform(
            post("/api/v1/sales-events/{id}/enabled", eventId)
                .with(userJwt(first))
                .contentType(MediaType.APPLICATION_JSON)
    ...
    mvc.perform(
            post("/api/v1/orders")
                ...
                .content(order))
        .andExpect(status().isBadRequest())
        .andExpect(jsonPath("$.code").value("SALES_EVENT_DISABLED"));
  }
  ...
  @Test
  void exactStockCorrectionDoesNotCreateInventorySales() throws Exception {
    Fixture fixture = fixture("stock-correction");
    Product product = product(fixture, "COUNTED", 500);
    mvc.perform(
            put("/api/v1/inventory/stock/{id}", product.getId())
    ...
    mvc.perform(
            put("/api/v1/inventory/stock/{id}", product.getId())
                ...
                .content("{\"quantity\":7}"))
        .andExpect(status().isBadRequest())
        .andExpect(jsonPath("$.code").value("STOCK_UNCHANGED"));
  }

  @Test
  void batchOrdersAndInventorySalesShareEventWithoutDoubleCountingRevenue() throws Exception {
    Fixture fixture = fixture("event-sales");
    Product product = product(fixture, "EVENT-SALE", 10);
    String eventJson =
        mvc.perform(
                post("/api/v1/sales-events")
                    .with(userJwt(fixture))
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(
                        "{\"name\":\"Paris Expo\",\"startDate\":\"2026-07-01\",\"endDate\":\"2026-07-05\"}"))
            .andExpect(status().isCreated())
            .andReturn()
            .getResponse()
            .getContentAsString();
    ...
    String batch =
        json.writeValueAsString(
            Map.of(
                ...
                "orders",
                List.of(Map.of("totalAmount", 10), Map.of("totalAmount", 20))));
    mvc.perform(
    ...
    String sale =
        json.writeValueAsString(
            Map.of(
                ...
                "EUR",
                "items",
                List.of(Map.of("productId", product.getId(), "quantity", 2, "unitPrice", 12.5))));
    mvc.perform(
            post("/api/v1/inventory/sales")
                ...
                .content(sale))
        .andExpect(status().isCreated())
        .andExpect(jsonPath("$.attributedDate").value("2026-07-05"))
    ...
    mvc.perform(
            get("/api/v1/reports/dashboard")
                .with(userJwt(fixture))
                .param("start", "2026-07-05")
                .param("end", "2026-07-05")
                .param("granularity", "HOUR"))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.currencies[0].netSales").value(30))
    ...
    mvc.perform(
            get("/api/v1/reports/inventory-sales")
                ...
                .param("end", "2026-07-05"))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.currencies[0].units").value(2))
  ...
  void orderLifecycleNeverChangesInventory() {
    ...
    OrderDtos.Request req =
        new OrderDtos.Request(
            List.of(
                new OrderDtos.ItemRequest(p.getId(), 3, null, BigDecimal.ZERO, BigDecimal.ZERO)),
  ...
  @Test
  void orderCreationDoesNotRequireAvailableStock() {
    Fixture f = fixture("stock");
    Product p = product(f, "LOW-1", 1);
    OrderDtos.Request req =
        new OrderDtos.Request(
            List.of(
            ...
            PaymentStatus.PAID,
            Instant.now());
    orderService.create(f.tenant.getId(), f.user.getId(), req);
    assertThat(products.findById(p.getId()).orElseThrow().getCurrentStock()).isEqualTo(1);
    assertThat(orders.existsByTenantId(f.tenant.getId())).isTrue();
  }

  @Test
  void batchOrderActionsPartiallySucceedAndRemainTenantScoped() throws Exception {
    Fixture first = fixture("batch-first"), second = fixture("batch-second");
    Product firstProduct = product(first, "BATCH", 10),
        secondProduct = product(second, "OTHER-BATCH", 5);
    SalesOrder legacyDraft = draft(first, firstProduct, 2);
    OrderDtos.Request otherRequest =
        new OrderDtos.Request(
            List.of(
    ...
    String confirm =
        json.writeValueAsString(Map.of("orderIds", List.of(legacyDraft.getId(), missing)));
    mvc.perform(
    ...
    String cancel =
        json.writeValueAsString(Map.of("orderIds", List.of(legacyDraft.getId(), otherOrder.id())));
    mvc.perform(
  ...
  @Test
  void productImagePresignUploadAndConfirmBindsThePrivateObject() throws Exception {
    Fixture f = fixture("image");
    Product p = product(f, "IMAGE", 2);
    byte[] image = "synthetic-png-test-content".getBytes(StandardCharsets.UTF_8);
    String checksum =
        HexFormat.of().formatHex(java.security.MessageDigest.getInstance("SHA-256").digest(image));
    String request =
        json.writeValueAsString(
            Map.of(
                "originalFilename",
  ...
  @Test
  void adminCanReadTenantsAndCreatesAudit() throws Exception {
    Fixture f = fixture("admin-target");
    User admin =
        users.save(
            new User(
                UUID.randomUUID(),
                ...
                "Admin",
                UserRole.ADMIN));
    mvc.perform(get("/api/v1/admin/tenants/{id}", f.tenant.getId()).with(adminJwt(admin)))
        .andExpect(status().isOk());
    mvc.perform(get("/api/v1/admin/reports/dashboard").with(adminJwt(admin)))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.defaultCurrency").value("MULTI"));
    assertThat(audits.findAll())
        .anyMatch(
            a ->
                a.getAction().equals("ADMIN_TENANT_READ")
                    && f.tenant.getId().equals(a.getTenantId()));
  }

  @Test
  void adminOperationalViewsFilterBySystemUserAndRejectRegularUsers() throws Exception {
    Fixture fixture = fixture("admin-operations");
    Product product = product(fixture, "ADMIN-OPS", 4);
    OrderDtos.Request request =
        new OrderDtos.Request(
            List.of(),
    ...
    User admin =
        users.save(
            new User(
                UUID.randomUUID(),
                null,
                "operations-admin-" + UUID.randomUUID(),
                "operations-admin-" + UUID.randomUUID() + "@test.local",
                passwords.encode("ValidPassword123!"),
                "Admin",
                UserRole.ADMIN));
    mvc.perform(
  ...
  @Test
  void disabledTenantBlocksLoginRefreshAndExistingAccessToken() throws Exception {
    ...
    var login =
        mvc.perform(
            ...
            .andReturn();
    var refreshCookie = Objects.requireNonNull(login.getResponse().getCookie("refresh_token"));
    f.tenant.setEnabled(false);
    tenants.saveAndFlush(f.tenant);
    mvc.perform(get("/api/v1/products").with(userJwt(f)))
        .andExpect(status().isForbidden())
  ...
  @Test
  void concurrentStockDecrementAllowsExactlyOneWinner() throws Exception {
    Fixture f = fixture("concurrent");
    Product p = product(f, "ONLY-ONE", 1);
    ExecutorService pool = Executors.newFixedThreadPool(2);
    CountDownLatch ready = new CountDownLatch(2), start = new CountDownLatch(1);
    Callable<Boolean> decrement =
        () -> {
          ready.countDown();
          ...
          try {
            inventory.apply(
                ...
                f.user.getId());
            return true;
          } catch (com.inventoryart.exception.BusinessException ex) {
            assertThat(ex.getCode()).isEqualTo("INSUFFICIENT_STOCK");
            return false;
          }
        };
    Future<Boolean> first = pool.submit(decrement), second = pool.submit(decrement);
  ...
  @Test
  void reportAndProductAggregatesRespectRefundedUnitsAndProductCost() throws Exception {
    Fixture f = fixture("report-cost");
    Product p = product(f, "REPORT-COST", 10);
    OrderDtos.Request request =
        new OrderDtos.Request(
            List.of(
                new OrderDtos.ItemRequest(p.getId(), 3, null, BigDecimal.ZERO, BigDecimal.ZERO)),
    ...
    User u =
        users.save(
            new User(
                ...
                prefix,
                UserRole.USER));
    return new Fixture(t, u);
  }

  private Product product(Fixture f, String sku, int stock) {
    Product p =
        products.save(
            new Product(
                UUID.randomUUID(),
  ...
  private SalesOrder draft(Fixture f, Product product, int quantity) {
    UUID id = UUID.randomUUID();
    BigDecimal total = new BigDecimal("10.00").multiply(BigDecimal.valueOf(quantity));
    SalesOrder order =
        new SalesOrder(
            id,
            f.tenant.getId(),
            "LEGACY-" + id.toString().substring(0, 8),
            OrderSource.MANUAL,
            OrderStatus.DRAFT,
            ...
            Instant.now(),
            f.user.getId());
    order.setAmounts(total, BigDecimal.ZERO, BigDecimal.ZERO, total, BigDecimal.ZERO);
  ...
  private org.springframework.security.test.web.servlet.request.SecurityMockMvcRequestPostProcessors
    ...
    return jwt()
        .jwt(
            j ->
                j.subject(f.user.getId().toString())
                    .claim("username", f.user.getUsername())
                    .claim("role", "USER")
                    .claim("tenantId", f.tenant.getId().toString()));
  }
  ...
  private org.springframework.security.test.web.servlet.request.SecurityMockMvcRequestPostProcessors
    ...
    return jwt()
        ...
        .jwt(
            j ->
                j.subject(admin.getId().toString())
