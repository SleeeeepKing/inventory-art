...
public class CurrentUserService {
    ...
    public CurrentUser get() {
        ...
        if (authentication == null || !(authentication.getPrincipal() instanceof Jwt jwt))
            throw new BusinessException("UNAUTHENTICATED", "Authentication required", HttpStatus.UNAUTHORIZED);
        try {
            ...
            if (tenantId != null && tenants.findById(tenantId).filter(t -> t.isEnabled()).isEmpty())
                throw new BusinessException("TENANT_DISABLED", "Tenant is disabled or no longer exists", HttpStatus.FORBIDDEN);
    ...
    public UUID tenantId() {
        ...
        if (user.tenantId() == null) throw new BusinessException("TENANT_REQUIRED", "Tenant context required", HttpStatus.FORBIDDEN);
        return user.tenantId();
    }
    public UUID userId() { return get().userId(); }
