...
public class JwtService {
    ...
    public JwtService(JwtEncoder encoder, AppProperties properties) { this.encoder = encoder; this.minutes = properties.getJwt().getAccessTokenMinutes(); }
    public IssuedToken issue(User user) {
        Instant now = Instant.now(); Instant expires = now.plusSeconds(minutes * 60);
        JwtClaimsSet.Builder claims = JwtClaimsSet.builder().issuer("inventory-art").issuedAt(now).expiresAt(expires)
