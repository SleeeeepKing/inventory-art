...
public class SumUpImportService {
  ...
  public SumUpImportService(
      ...
      SumUpFileParser parser,
      CurrentUserService currentUser,
      ObjectProvider<SumUpImportCommitter> committer,
      JdbcTemplate jdbc,
      ObjectMapper objectMapper,
      EntityManager entityManager,
      AuditService audit,
      SalesEventService salesEvents,
    ...
    this.legacySpreadsheetMaxFileSize =
        properties.getImportConfig().getLegacySpreadsheetMaxFileSize();
  ...
  @Transactional
  public SumUpDtos.BatchResponse upload(
      MultipartFile file, ImportType requestedType, UUID eventId) {
    ...
    batches
        .findByTenantIdAndSourceProviderAndFileChecksum(tenantId, "SUMUP", checksum)
        .ifPresent(
            existing -> {
              throw new BusinessException(
                  "DUPLICATE_IMPORT_FILE",
                  "This file has already been uploaded as batch " + existing.getId(),
                  HttpStatus.CONFLICT);
            });
    ...
    try (InputStream input = file.getInputStream()) {
      ...
      throw new BusinessException(
          "IMPORT_UPLOAD_FAILED", "Unable to store import file", HttpStatus.BAD_GATEWAY);
    }
    try {
      ImportBatch batch =
      ...
      if (exception instanceof DataIntegrityViolationException) {
        throw new BusinessException(
            "DUPLICATE_IMPORT_FILE", "This file has already been uploaded", HttpStatus.CONFLICT);
      }
      throw exception;
    }
  }
  ...
  private static void setBatchId(ImportBatch batch, UUID id) {
    batch.assignId(id);
  }

  @Transactional(readOnly = true)
  public PageResponse<SumUpDtos.BatchResponse> list(Pageable pageable) {
    Page<SumUpDtos.BatchResponse> page =
        batches
            .findAllByTenantId(currentUser.tenantId(), pageable)
            .map(SumUpDtos.BatchResponse::from);
    return PageResponse.of(page);
  }

  @Transactional(readOnly = true)
  public SumUpDtos.BatchResponse get(UUID batchId) {
    return SumUpDtos.BatchResponse.from(required(batchId));
  }
  ...
  public SumUpDtos.AnalyzeResponse analyze(UUID batchId) {
      ...
      metadata =
          parser.parse(
              ...
              (rowNumber, source) -> {
                if (++seen[0] > maxRows) {
                  throw new BusinessException(
                      "IMPORT_ROW_LIMIT_EXCEEDED", "Import file exceeds the configured row limit");
                }
                source
                    .keySet()
                    .forEach(
                ...
                ImportType rowType =
                    batch.getImportType() == ImportType.UNKNOWN
                        ? SumUpNormalizer.detectType(discoveredMappings.values())
                        : batch.getImportType();
    ...
    metadata
        ...
        .forEach(
            header -> {
              ...
              if (target != null)
                columnMappings.save(
                    ImportColumnMapping.of(batch.getTenantId(), batchId, header, target));
            });
    ImportType detected =
        batch.getImportType() == ImportType.UNKNOWN
            ? SumUpNormalizer.detectType(discoveredMappings.values())
    ...
    batch.analysisComplete(
        detected,
        metadata.encoding(),
        metadata.delimiter(),
        metadata.emittedRows(),
        valid[0],
        errors[0],
        mappingRequired);
    ...
    audit.record(
        batch.getTenantId(),
        "SUMUP_IMPORT_ANALYZE",
        "IMPORT_BATCH",
        batchId,
        "SUCCESS",
        Map.of("rows", metadata.emittedRows(), "errors", errors[0]));
    return new SumUpDtos.AnalyzeResponse(
        SumUpDtos.BatchResponse.from(savedBatch),
        metadata.headers(),
        Map.copyOf(discoveredMappings));
  }

  @Transactional
  public SumUpDtos.BatchResponse applyColumnMapping(
      UUID batchId, SumUpDtos.ColumnMappingRequest request) {
    ImportBatch batch = required(batchId);
    ...
    request
        ...
        .forEach(
            (source, target) ->
                columnMappings.save(
                    ImportColumnMapping.of(batch.getTenantId(), batchId, source, target)));
    ZoneId zone = tenantZone(batch.getTenantId());
    ImportType type =
        batch.getImportType() == ImportType.UNKNOWN
            ? SumUpNormalizer.detectType(request.mappings().values())
            : batch.getImportType();
    int valid = 0;
    int errors = 0;
    ProductMatchIndex productIndex = productMatchIndex(batch.getTenantId());
    ...
    while (true) {
      Page<ImportRow> page =
          rows.findAllByTenantIdAndImportBatchId(
              batch.getTenantId(),
              batchId,
      ...
      for (ImportRow row : page.getContent()) {
        ...
        try {
          normalized =
              SumUpNormalizer.normalize(row.getSanitizedRawData(), request.mappings(), zone);
  ...
  public SumUpDtos.PreviewResponse preview(UUID batchId) {
    ...
    List<SumUpDtos.RowResponse> previewRows =
        rows
            .findAllByTenantIdAndImportBatchId(
                batch.getTenantId(), batchId, PageRequest.of(0, 20, Sort.by("rowNumber")))
            .stream()
    ...
    return new SumUpDtos.PreviewResponse(
        SumUpDtos.BatchResponse.from(batch),
        previewRows,
        Math.max(0, batch.getValidRows() - duplicateCount),
        duplicateCount,
        batch.getErrorRows(),
  ...
  @Transactional
  public SumUpDtos.BatchResponse saveProductMappings(
      UUID batchId, SumUpDtos.ProductMappingsRequest request) {
    ImportBatch batch = required(batchId);
    verifyVersion(batch, request.expectedAnalysisVersion());
    ...
    for (SumUpDtos.ProductMappingItem item : request.mappings()) {
      if ((item.externalProductName() == null || item.externalProductName().isBlank())
          && (item.externalProductReference() == null
              || item.externalProductReference().isBlank())) {
        throw new BusinessException(
            "INVALID_PRODUCT_MAPPING", "Product name or reference is required");
      }
      Integer exists =
          jdbc.queryForObject(
      ...
      selections.add(
          new ProductSelection(
              normalized(item.externalProductReference()),
              normalized(item.externalProductName()),
              item.productId()));
      if (!item.remember()) continue;
      String key =
          SumUpNormalizer.normalizeText(
              item.externalProductName() == null || item.externalProductName().isBlank()
                  ? item.externalProductReference()
    ...
    while (true) {
      ...
      for (ImportRow row : page.getContent()) {
        ...
        selections.stream()
            .filter(selection -> selection.matches(sku, name))
            .findFirst()
  ...
  public SumUpDtos.ImportActionResponse reverse(UUID batchId) {
    ...
    if (integration == null) {
      throw new BusinessException(
          "IMPORT_COMMITTER_NOT_AVAILABLE",
          "Order import reverse integration is not installed",
          HttpStatus.NOT_IMPLEMENTED);
    }
    integration.reverse(
        new SumUpImportCommitter.ReverseCommand(
            batch.getTenantId(), currentUser.userId(), batchId));
    batch.markReversed(currentUser.userId());
    audit.record(
        batch.getTenantId(), "SUMUP_IMPORT_REVERSE", "IMPORT_BATCH", batchId, "SUCCESS", Map.of());
    return actionResponse(batch);
  }
  ...
  public PageResponse<SumUpDtos.RowResponse> rowPage(UUID batchId, Pageable pageable) {
    ImportBatch batch = required(batchId);
    Page<SumUpDtos.RowResponse> page =
        rows.findAllByTenantIdAndImportBatchId(batch.getTenantId(), batchId, pageable)
            .map(SumUpDtos.RowResponse::from);
    return PageResponse.of(page);
  }
  ...
  public void writeErrors(UUID batchId, Writer output) throws IOException {
    ImportBatch batch = required(batchId);
    List<ImportRow> errorRows =
        rows.findAllByTenantIdAndImportBatchIdAndProcessingStatusInOrderByRowNumber(
    ...
    try (CSVPrinter csv =
        new CSVPrinter(
            ...
            CSVFormat.DEFAULT
                .builder()
                .setHeader("rowNumber", "status", "errors", "sanitizedRawData")
                .get())) {
  ...
  private ImportBatch required(UUID batchId) {
    return batches
        .findByIdAndTenantId(batchId, currentUser.tenantId())
        .orElseThrow(() -> new NotFoundException("Import batch"));
  }

  private ZoneId tenantZone(UUID tenantId) {
    String timezone =
        jdbc.queryForObject("select timezone from tenants where id = ?", String.class, tenantId);
    try {
  ...
  private static void ensureAnalyzed(ImportBatch batch) {
    if (batch.getStatus() == ImportBatchStatus.UPLOADED
        || batch.getStatus() == ImportBatchStatus.ANALYZING
        || batch.getStatus() == ImportBatchStatus.FAILED)
      throw SumUpExceptions.invalidState(batch.getStatus(), "be previewed");
  }
    ...
    mappings.forEach(
        (source, target) -> {
          if (!SumUpNormalizer.TARGET_FIELDS.contains(target)) {
            throw new BusinessException(
                "INVALID_IMPORT_TARGET_FIELD", "Unsupported target field: " + target);
          }
          if (!targets.add(target)) {
            throw new BusinessException(
                "DUPLICATE_IMPORT_TARGET_FIELD",
                "A target field can only be mapped once: " + target);
          }
        });
  }
  ...
  private void validateUpload(MultipartFile file, String filename) {
    if (file.isEmpty() || file.getSize() <= 0)
      throw new BusinessException("EMPTY_IMPORT_FILE", "Import file is empty");
    if (file.getSize() > maxFileSize)
      throw new BusinessException("IMPORT_FILE_TOO_LARGE", "Import file exceeds configured limit");
    ...
    if (!csv && !xls && !xlsx) {
      throw new BusinessException(
          "UNSUPPORTED_IMPORT_FILE", "Only CSV, XLS and XLSX files are supported");
    }
    if (xls && file.getSize() > legacySpreadsheetMaxFileSize) {
      throw new BusinessException(
          "LEGACY_SPREADSHEET_TOO_LARGE",
  ...
  private static String safeFilename(String source) {
    ...
    filename =
        filename.substring(filename.lastIndexOf('/') + 1).replaceAll("[\\p{Cntrl}]", "").trim();
    if (filename.isBlank() || !filename.contains(".")) {
      throw new BusinessException(
          "INVALID_IMPORT_FILENAME", "Import filename must include an extension");
    }
  ...
  private static SumUpDtos.ImportActionResponse actionResponse(ImportBatch batch) {
    return new SumUpDtos.ImportActionResponse(
        batch.getId(),
        batch.getStatus(),
  ...
  private ProductMatchIndex productMatchIndex(UUID tenantId) {
    ...
    jdbc.query(
        "select id,sku,name from products where tenant_id=? and enabled=true",
        rs -> {
  ...
  private record ProductMatchIndex(
      Map<String, UUID> external, Map<String, UUID> skus, Map<String, UUID> names) {}
  ...
  private record ProductSelection(String reference, String name, UUID productId) {
    boolean matches(String rowReference, String rowName) {
      return (reference != null && reference.equals(rowReference))
          || (name != null && name.equals(rowName));
    }
  }
}
