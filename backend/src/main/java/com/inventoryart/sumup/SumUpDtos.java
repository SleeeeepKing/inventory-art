...
public final class SumUpDtos {
  ...
  public record BatchResponse(
    ...
    static BatchResponse from(ImportBatch batch) {
      return new BatchResponse(
          ...
          batch.getEventId(),
          batch.getEventName(),
          batch.getDetectedEncoding(),
          batch.getDetectedDelimiter(),
          batch.getAnalysisVersion(),
          batch.getStatus(),
          batch.getTotalRows(),
          batch.getValidRows(),
  ...
  public record AnalyzeResponse(
      BatchResponse batch, List<String> sourceColumns, Map<String, String> suggestedMappings) {}

  public record ColumnMappingRequest(
      @Min(0) int expectedAnalysisVersion,
  ...
  public record RowResponse(
    ...
    static RowResponse from(ImportRow row) {
      return new RowResponse(
          ...
          row.getExternalTransactionId(),
          row.getFingerprint(),
          row.getSanitizedRawData(),
          row.getNormalizedData(),
          row.getValidationErrors(),
          row.getLinkedOrderId(),
          row.getLinkedProductId());
    }
  }
  ...
  public record PreviewResponse(
      ...
      int errors,
      int needsProductMapping,
      boolean createsOrders,
      boolean affectsInventory,
      boolean financialOnly) {}

  public record ProductMappingItem(
      String externalProductReference,
      String externalProductName,
      @NotNull UUID productId,
      boolean remember) {}

  public record ProductMappingsRequest(
      @Min(0) int expectedAnalysisVersion, @NotEmpty List<@Valid ProductMappingItem> mappings) {}

  public record ConfirmRequest(@Min(0) int expectedAnalysisVersion) {}

  public record EventRequest(@NotNull UUID eventId) {}

  public record ImportActionResponse(
      UUID batchId,
      ImportBatchStatus status,
      int importedRows,
      int updatedRows,
