...
final class SumUpNormalizer {
    ...
    private static final Pattern PAN_CANDIDATE = Pattern.compile("(?<!\\d)(?:\\d[ -]?){12,18}\\d(?!\\d)");
    ...
    static Map<String, Object> sanitize(Map<String, String> raw) {
        ...
        raw.forEach((header, sourceValue) -> {
            String key = normalizeHeader(header);
            if (key.contains("cvv") || key.contains("cvc") || key.contains("cryptogramme")
                || key.contains("security code") || key.contains("code securite")) return;
    ...
    static Map<String, Object> normalize(Map<String, Object> sanitized, Map<String, String> mappings, ZoneId zone) {
        Map<String, String> mapped = new LinkedHashMap<>();
        sanitized.forEach((source, value) -> {
            String target = mappings.get(source);
            if (target == null) target = suggestedTarget(source);
            if (target != null && value != null && !value.toString().isBlank()) mapped.put(target, value.toString().trim());
        });
    ...
    static ImportType detectType(Iterable<String> targets) {
        ...
        if (targetSet.contains("productName") && targetSet.contains("quantity")) {
            return targetSet.contains("transactionId") || targetSet.contains("transactionCode")
                ? ImportType.ORDER_HISTORY : ImportType.PRODUCT_SALES;
        }
        if (targetSet.contains("tax") && (targetSet.contains("grossRevenue") || targetSet.contains("netRevenue"))
            && !targetSet.contains("transactionId")) return ImportType.ACCOUNTING_REPORT;
        if (targetSet.contains("amount") && (targetSet.contains("transactionId") || targetSet.contains("status"))) {
    ...
    static String fingerprint(java.util.UUID tenantId, Map<String, Object> normalized) {
        String canonical = String.join("|",
            tenantId.toString(), value(normalized, "occurredAt"), value(normalized, "amount"),
            value(normalized, "currency"), value(normalized, "type"),
            normalizeText(value(normalized, "description")), normalizeText(value(normalized, "merchant")));
    ...
    private static Object normalizeField(String field, String value) {
        return switch (field) {
            case "amount", "feeAmount", "netAmount", "refundAmount", "unitPrice", "discount",
                 "revenue", "grossRevenue", "netRevenue", "tax" -> parseDecimal(value);
    ...
    private static BigDecimal parseDecimal(String raw) {
        ...
        if (comma >= 0 && dot >= 0) {
            ...
            value = digitsAfter == 3 && value.indexOf(',') == comma ? value.replace(",", "") : value.replace(',', '.');
        } else if (dot >= 0 && value.length() - dot - 1 == 3 && value.indexOf('.') == dot) {
            value = value.replace(".", "");
        }
        if (value.isBlank() || value.equals("-")) throw new IllegalArgumentException("Invalid decimal value");
    ...
    private static Instant parseDateTime(String date, String time, ZoneId zone) {
        ...
        try { return Instant.parse(date.trim()); } catch (DateTimeParseException ignored) {}
        String combined = date.trim() + (time == null || time.isBlank() ? "" : " " + time.trim());
        for (DateTimeFormatter format : DATE_TIME_FORMATS) {
            try { return LocalDateTime.parse(combined, format).atZone(zone).toInstant(); }
        ...
        for (DateTimeFormatter format : DATE_FORMATS) {
            try {
                ...
                if (time != null && !time.isBlank()) {
                    try { localTime = LocalTime.parse(time.trim(), DateTimeFormatter.ISO_LOCAL_TIME); }
                    catch (DateTimeParseException ignored) {}
                }
    ...
    private static String maskPanCandidates(String source) {
        ...
        while (matcher.find()) {
            ...
            if (luhn(digits)) matcher.appendReplacement(result, Matcher.quoteReplacement("****" + digits.substring(digits.length() - 4)));
        }
    ...
    private static void require(Map<String, Object> values, String key, List<String> errors) {
        if (!values.containsKey(key) || value(values, key).isBlank()) errors.add("MISSING_" + key.toUpperCase(Locale.ROOT));
    }

    private static void requireOne(Map<String, Object> values, List<String> keys, List<String> errors) {
    ...
    private static Map<String, String> aliases() {
        ...
        aliases(map, "transactionId", "transaction id", "transaction identifier", "id transaction", "identifiant transaction");
        aliases(map, "transactionCode", "transaction code", "code transaction", "ticket", "order id", "numero commande");
        ...
        aliases(map, "refundAmount", "refund", "refund amount", "remboursement", "montant remboursement");
        ...
        aliases(map, "payoutReference", "payout reference", "reference versement", "reference virement");
