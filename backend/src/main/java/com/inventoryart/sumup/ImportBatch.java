...
public class ImportBatch {
    ...
    @Column(name = "source_provider", nullable = false) private String sourceProvider;
    @Enumerated(EnumType.STRING) @Column(name = "import_type", nullable = false) private ImportType importType;
    @Column(name = "original_filename", nullable = false) private String originalFilename;
    @Column(name = "stored_object_key", nullable = false) private String storedObjectKey;
    @Column(name = "file_checksum", nullable = false, length = 64) private String fileChecksum;
    @Column(name = "file_size", nullable = false) private long fileSize;
    @Column(name = "detected_encoding") private String detectedEncoding;
    @Column(name = "detected_delimiter") private String detectedDelimiter;
    ...
    @Column(name = "updated_rows", nullable = false) private int updatedRows;
    @Column(name = "duplicate_rows", nullable = false) private int duplicateRows;
    @Column(name = "skipped_rows", nullable = false) private int skippedRows;
    ...
    void beginAnalysis() {
        if (status != ImportBatchStatus.UPLOADED && status != ImportBatchStatus.READY_FOR_MAPPING
            && status != ImportBatchStatus.READY_FOR_CONFIRMATION && status != ImportBatchStatus.FAILED) {
    ...
    void analysisComplete(ImportType detectedType, String encoding, String delimiter, int total,
                          int valid, int errors, boolean mappingRequired) {
        ...
        errorRows = errors;
        analysisVersion++;
        status = mappingRequired ? ImportBatchStatus.READY_FOR_MAPPING : ImportBatchStatus.READY_FOR_CONFIRMATION;
    }
    ...
    void markReversed(UUID userId) { status = ImportBatchStatus.REVERSED; reversedAt = Instant.now(); reversedBy = userId; }
    ...
    public String getStoredObjectKey() { return storedObjectKey; }
