...
public class AdminDataController {
  ...
  private final ProductSalesService productSales;
  private final AdminDataQueryService data;
  private final SalesEventRepository events;

  public AdminDataController(
      ProductRepository products,
      AuditService audit,
      ProductSalesService productSales,
      AdminDataQueryService data,
  ...
  @GetMapping("/products")
  public PageResponse<ProductController.Response> products(
      @RequestParam(required = false) UUID tenantId,
      @RequestParam(defaultValue = "0") int page,
      @RequestParam(defaultValue = "20") int size) {
    Pageable p =
        PageRequest.of(page, Math.min(size, 50), Sort.by(Sort.Direction.DESC, "updatedAt"));
    Page<Product> result =
        tenantId == null ? products.findAll(p) : products.findAllByTenantId(tenantId, p);
    var summaries = productSales.summarize(result.getContent());
    audit.record(tenantId, "ADMIN_PRODUCT_LIST", "PRODUCT", null, "SUCCESS", Map.of("page", page));
    return PageResponse.of(
        result.map(
            product ->
                ProductController.Response.from(
                    product,
                    null,
  ...
  @GetMapping("/orders")
  public PageResponse<AdminDataQueryService.AdminOrderRow> orders(
      @RequestParam(required = false) UUID tenantId,
  ...
  public PageResponse<AdminDataQueryService.AdminInventoryRow> inventory(
      @RequestParam(required = false) UUID tenantId,
      @RequestParam(required = false) UUID userId,
      @RequestParam(required = false) @DateTimeFormat(iso = DateTimeFormat.ISO.DATE_TIME)
          Instant from,
  ...
  @GetMapping("/sales-events")
  public List<AdminEventRow> salesEvents(@RequestParam UUID tenantId) {
    return events.findAllByTenantIdOrderByNameAsc(tenantId).stream()
        .map(
            event ->
                new AdminEventRow(
                    event.getId(),
  ...
  public record AdminEventRow(
      UUID id, String name, LocalDate startDate, LocalDate endDate, boolean enabled) {}
}
