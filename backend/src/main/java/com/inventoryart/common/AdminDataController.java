...
public class AdminDataController {
    private final ProductRepository products;private final OrderRepository orders;private final OrderService orderService;private final AuditService audit;private final ProductSalesService productSales;
    public AdminDataController(ProductRepository products,OrderRepository orders,OrderService orderService,AuditService audit,ProductSalesService productSales){this.products=products;this.orders=orders;this.orderService=orderService;this.audit=audit;this.productSales=productSales;}
    @GetMapping("/products") public PageResponse<ProductController.Response> products(@RequestParam(required=false)UUID tenantId,@RequestParam(defaultValue="0")int page,@RequestParam(defaultValue="20")int size){Pageable p=PageRequest.of(page,Math.min(size,100),Sort.by(Sort.Direction.DESC,"updatedAt"));Page<Product> result=tenantId==null?products.findAll(p):products.findAllByTenantId(tenantId,p);var summaries=productSales.summarize(result.getContent());audit.record(tenantId,"ADMIN_PRODUCT_LIST","PRODUCT",null,"SUCCESS",Map.of("page",page));return PageResponse.of(result.map(product->ProductController.Response.from(product,null,summaries.getOrDefault(product.getId(),ProductSalesService.Summary.EMPTY))));}
