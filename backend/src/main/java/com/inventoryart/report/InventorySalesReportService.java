...
public class InventorySalesReportService {
    ...
    public InventorySalesReportService(NamedParameterJdbcTemplate jdbc, CurrentUserService currentUser) {
        this.jdbc = jdbc;
    ...
    @Transactional(readOnly = true)
    public ReportDtos.InventorySalesReport adminReport(UUID tenantId, LocalDate start, LocalDate end) {
    ...
    private ReportDtos.InventorySalesReport report(UUID tenantId, LocalDate requestedStart,
                                                    LocalDate requestedEnd, boolean admin) {
        ...
        List<ReportDtos.InventorySalesMetrics> currencies = jdbc.query("""
            ...
            """ + BASE + " group by b.currency order by b.currency", params,
            (rs, row) -> new ReportDtos.InventorySalesMetrics(rs.getString("currency"),
                rs.getLong("units"), rs.getLong("batches"), money(rs.getBigDecimal("attributed_amount")),
                money(rs.getBigDecimal("weighted_average")), money(rs.getBigDecimal("minimum_price")),
                money(rs.getBigDecimal("maximum_price"))));
        return new ReportDtos.InventorySalesReport(start, end, settings.zone().getId(), currencies,
            groups("p.id", "p.sku", "p.name", "p.id,p.sku,p.name,b.currency", params),
            groups("null::uuid", "null::varchar", "b.sales_channel", "b.sales_channel,b.currency", params),
            groups("null::uuid", "null::varchar", "coalesce(b.event_name,'')", "b.event_name,b.currency", params));
    }

    private List<ReportDtos.InventorySalesGroup> groups(String productId, String sku, String label,
                                                         String groupBy, MapSqlParameterSource params) {
        return jdbc.query("""
            select %s product_id,%s sku,%s label,b.currency,
                   ...
                   sum(abs(m.quantity)*m.unit_price)/nullif(sum(abs(m.quantity)),0) weighted_average,
                   min(m.unit_price) minimum_price,max(m.unit_price) maximum_price
            """.formatted(productId, sku, label) + BASE
            + " group by " + groupBy + " order by attributed_amount desc,label limit 100", params,
            (rs, row) -> new ReportDtos.InventorySalesGroup(rs.getObject("product_id", UUID.class),
                rs.getString("sku"), rs.getString("label"), rs.getString("currency"),
                rs.getLong("units"), rs.getLong("batches"), money(rs.getBigDecimal("attributed_amount")),
                money(rs.getBigDecimal("weighted_average")), money(rs.getBigDecimal("minimum_price")),
                money(rs.getBigDecimal("maximum_price"))));
    }
    ...
    private Settings settings(UUID tenantId, boolean admin) {
        ...
        List<Settings> result = jdbc.query("select timezone from tenants where id=:id", Map.of("id", tenantId),
    ...
    private static ZoneId safeZone(String value) {
        try { return ZoneId.of(value); } catch (RuntimeException ignored) { return ZoneId.of("UTC"); }
    }
