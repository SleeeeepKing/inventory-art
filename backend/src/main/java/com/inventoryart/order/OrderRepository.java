...
public interface OrderRepository extends JpaRepository<SalesOrder,UUID>{
    ...
    @Query("select o from SalesOrder o where o.tenantId=:tenantId and (:status is null or o.status=:status) and (:source is null or o.source=:source) and (:channel is null or o.salesChannel=:channel) and (:from is null or o.orderDate>=:from) and (:to is null or o.orderDate<:to) and (:q is null or lower(o.orderNumber) like lower(concat('%',:q,'%')) or lower(coalesce(o.eventName,'')) like lower(concat('%',:q,'%')))")
    Page<SalesOrder> search(UUID tenantId,String q,OrderStatus status,OrderSource source,SalesChannel channel,Instant from,Instant to,Pageable pageable);
