...
public class OrderService {
    ...
    @Transactional public OrderDtos.Response update(UUID tenant,UUID user,UUID id,OrderDtos.Request request){
        ...
        items.deleteAllByTenantIdAndOrderId(tenant,id);items.flush();items.saveAll(replacement);order.updateDetails(request.salesChannel(),request.eventName(),request.customerName(),request.customerEmail(),request.customerNote(),request.paymentMethod(),request.paymentStatus(),request.orderDate());setTotals(order,replacement);return response(order,replacement);
    }
    @Transactional public OrderDtos.Response allocate(UUID tenant,UUID user,UUID id,OrderDtos.Request request){
        ...
        order.updateDetails(request.salesChannel(),request.eventName(),request.customerName(),request.customerEmail(),request.customerNote(),request.paymentMethod(),request.paymentStatus(),request.orderDate());
    ...
    public OrderDtos.Response response(SalesOrder o,List<OrderItem> lines){return new OrderDtos.Response(o.getId(),o.getOrderNumber(),o.getSource().name(),o.getStatus().name(),o.getAllocationStatus().name(),o.getSalesChannel().name(),o.getEventName(),o.getCustomerName(),o.getCustomerEmail(),o.getCustomerNote(),o.getCurrency(),o.getSubtotal(),o.getDiscountAmount(),o.getTaxAmount(),o.getRefundAmount(),o.getTotalAmount(),o.getUnallocatedAmount(),o.getPaymentMethod().name(),o.getPaymentStatus().name(),o.getOrderDate(),o.isInventoryApplied(),o.getVersion(),lines.stream().map(i->new OrderDtos.ItemResponse(i.getId(),i.getProductId(),i.getProductSkuSnapshot(),i.getProductNameSnapshot(),i.getUnitPrice(),i.getQuantity(),i.getDiscountAmount(),i.getTaxRate(),i.getTaxAmount(),i.getLineTotal(),i.getRefundedQuantity())).toList(),o.getCreatedAt(),o.getUpdatedAt());}
