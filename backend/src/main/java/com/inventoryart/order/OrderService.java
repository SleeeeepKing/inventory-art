...
public class OrderService {
    ...
    private final OrderRepository orders;private final OrderItemRepository items;private final ProductRepository products;private final OrderRefundRepository refunds;private final OrderRefundItemRepository refundItems;private final PaymentRepository payments;private final SalesEventService events;
    public OrderService(OrderRepository orders,OrderItemRepository items,ProductRepository products,OrderRefundRepository refunds,OrderRefundItemRepository refundItems,PaymentRepository payments,SalesEventService events){this.orders=orders;this.items=items;this.products=products;this.refunds=refunds;this.refundItems=refundItems;this.payments=payments;this.events=events;}
    ...
    @Transactional public OrderDtos.BatchCreateResponse createBatch(UUID tenant,UUID user,OrderDtos.BatchCreateRequest request){
        SalesEvent event=events.requiredEnabled(tenant,request.eventId());
        List<OrderDtos.BatchSuccess> created=new ArrayList<>(request.orders().size());BigDecimal total=BigDecimal.ZERO;
        for(OrderDtos.BatchCreateLine line:request.orders()){
            BigDecimal amount=money(line.totalAmount());
            if(amount.signum()<=0)throw new BusinessException("INVALID_ORDER_AMOUNT","Order amount must be positive");
            SalesOrder order=orders.save(new SalesOrder(UUID.randomUUID(),tenant,number(),OrderSource.MANUAL,OrderStatus.DRAFT,AllocationStatus.FULLY_ALLOCATED,SalesChannel.EXHIBITION,event.getId(),event.getName(),null,null,null,request.currency().toUpperCase(Locale.ROOT),request.paymentMethod(),request.paymentStatus(),request.orderDate(),user));
            setRecordedTotal(order,amount);order.confirmed();recordManualPaymentIfPaid(order);
            created.add(new OrderDtos.BatchSuccess(order.getId(),order.getOrderNumber(),order.getStatus().name()));total=total.add(amount);
        }
        return new OrderDtos.BatchCreateResponse(event.getId(),event.getName(),request.currency().toUpperCase(Locale.ROOT),created.size(),money(total),created);
    }
    ...
    public OrderDtos.Response response(SalesOrder o,List<OrderItem> lines){return new OrderDtos.Response(o.getId(),o.getOrderNumber(),o.getSource().name(),o.getStatus().name(),o.getAllocationStatus().name(),o.getSalesChannel().name(),o.getEventId(),o.getEventName(),o.getCustomerName(),o.getCustomerEmail(),o.getCustomerNote(),o.getCurrency(),o.getSubtotal(),o.getDiscountAmount(),o.getTaxAmount(),o.getRefundAmount(),o.getTotalAmount(),o.getUnallocatedAmount(),o.getPaymentMethod().name(),o.getPaymentStatus().name(),o.getOrderDate(),o.getCreatedBy(),o.getVersion(),lines.stream().map(i->new OrderDtos.ItemResponse(i.getId(),i.getProductId(),i.getProductSkuSnapshot(),i.getProductNameSnapshot(),i.getUnitPrice(),i.getQuantity(),i.getDiscountAmount(),i.getTaxRate(),i.getTaxAmount(),i.getLineTotal(),i.getRefundedQuantity())).toList(),o.getCreatedAt(),o.getUpdatedAt());}
