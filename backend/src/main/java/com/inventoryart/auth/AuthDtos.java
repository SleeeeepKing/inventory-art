...
public final class AuthDtos {
    ...
    public record TenantSummary(UUID id, String name, String defaultCurrency, String timezone, String locale) {
        public static TenantSummary from(Tenant tenant) {
            return tenant == null ? null : new TenantSummary(tenant.getId(), tenant.getName(),
                tenant.getDefaultCurrency(), tenant.getTimezone(), tenant.getLocale());
        }
    }
    public record UserResponse(UUID id, UUID tenantId, String username, String email, String displayName,
                               String role, String preferredLocale, boolean enabled, TenantSummary tenant) {
        public static UserResponse from(User user, Tenant tenant) {
            return new UserResponse(user.getId(), user.getTenantId(), user.getUsername(), user.getEmail(),
                user.getDisplayName(), user.getRole().name(), user.getPreferredLocale(), user.isEnabled(),
