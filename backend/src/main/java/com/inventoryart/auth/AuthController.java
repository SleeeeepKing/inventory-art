...
public class AuthController {
  ...
  public ResponseEntity<AuthDtos.AuthResponse> login(
    ...
    AuthService.Session session =
        service.login(
            request.username(), request.password(), clientIp(http), http.getHeader("User-Agent"));
    return response(session);
  }

  @PostMapping("/refresh")
  public ResponseEntity<AuthDtos.AuthResponse> refresh(
      @CookieValue(name = COOKIE, required = false) String token, HttpServletRequest http) {
    validateOrigin(http);
    ...
    return ResponseEntity.ok()
        .header(
            ...
            cookie(
                    ...
                    Duration.ofDays(properties.getJwt().getRefreshTokenDays()))
                .toString())
  ...
  private void validateOrigin(HttpServletRequest request) {
    ...
    if (origin != null
        && Arrays.stream(properties.getSecurity().getCorsAllowedOrigins().split(","))
            .map(String::trim)
            .noneMatch(origin::equals))
      throw new com.inventoryart.exception.BusinessException(
          "INVALID_ORIGIN", "Origin is not allowed", org.springframework.http.HttpStatus.FORBIDDEN);
  }
}
