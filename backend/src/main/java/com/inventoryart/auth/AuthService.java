...
public class AuthService {
  ...
  public Session login(String username, String password, String ip, String agent) {
    ...
    String key = ip + ":" + normalized;
    limiter.check(key);
    User user =
        users
            .findByUsernameIgnoreCase(normalized)
            .filter(User::isEnabled)
            .filter(this::tenantIsEnabled)
    ...
    audits.save(
        new AuditLog(
            ...
            sanitizeAgent(agent),
            java.util.Map.of()));
    return newSession(user, UUID.randomUUID(), ip, agent);
  }
  ...
  public Session refresh(String raw, String ip, String agent) {
    ...
    RefreshToken existing =
        tokens
            .findLockedByTokenHash(hash(raw))
            .orElseThrow(
                () ->
  ...
  public void revokeUser(UUID userId) {
    tokens.revokeUser(userId, Instant.now());
  }

  private Session newSession(User user, UUID family, String ip, String agent) {
    return newSession(user, family, UUID.randomUUID(), ip, agent);
  }
  ...
  private Session newSession(User user, UUID family, UUID tokenId, String ip, String agent) {
    ...
    tokens.save(
        new RefreshToken(
            ...
            family,
            hash(raw),
            Instant.now().plusSeconds(properties.getJwt().getRefreshTokenDays() * 86400),
            ip,
            sanitizeAgent(agent)));
    JwtService.IssuedToken access = jwt.issue(user);
    return new Session(
        new AuthDtos.AuthResponse(access.value(), access.expiresAt(), userResponse(user)), raw);
  }
  ...
  private String sanitizeAgent(String agent) {
    return agent == null ? null : agent.substring(0, Math.min(agent.length(), 500));
  }

  private boolean tenantIsEnabled(User user) {
    return user.getTenantId() == null
        || tenants.findById(user.getTenantId()).filter(t -> t.isEnabled()).isPresent();
  }
