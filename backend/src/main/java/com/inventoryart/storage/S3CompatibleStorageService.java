...
public final class S3CompatibleStorageService implements StorageService, AutoCloseable {
    ...
    public PresignedRequest presignPut(String objectKey, String contentType, long contentLength,
        ...
        PutObjectRequest put = PutObjectRequest.builder()
            ...
            .metadata(Map.of("sha256", checksumSha256)).build();
        PresignedPutObjectRequest signed = presigner.presignPutObject(PutObjectPresignRequest.builder()
            .signatureDuration(validity).putObjectRequest(put).build());
        Map<String, String> headers = new LinkedHashMap<>();
        signed.signedHeaders().forEach((name, values) -> {
            if (!values.isEmpty() && !name.equalsIgnoreCase("host")) headers.put(name, values.getFirst());
        });
        return new PresignedRequest(signed.url().toString(), Map.copyOf(headers), Instant.now().plus(validity));
    }
    ...
    public void put(String objectKey, InputStream content, long contentLength, String contentType,
        ...
        try {
            client.putObject(PutObjectRequest.builder().bucket(bucket).key(objectKey).contentType(contentType)
                .contentLength(contentLength).metadata(metadata).build(), RequestBody.fromInputStream(content, contentLength));
    ...
    public ObjectMetadata head(String objectKey) {
        try {
            HeadObjectResponse response = client.headObject(HeadObjectRequest.builder().bucket(bucket).key(objectKey).build());
            return new ObjectMetadata(response.contentLength(), response.contentType(), response.metadata().get("sha256"),
                response.metadata());
        } catch (NoSuchKeyException exception) {
            throw new BusinessException("FILE_NOT_FOUND", "Stored object not found", HttpStatus.NOT_FOUND);
        } catch (RuntimeException exception) {
    ...
    private BusinessException storageFailure(String operation, RuntimeException cause) {
        return new BusinessException("STORAGE_PROVIDER_ERROR", "Unable to " + operation + " private object", HttpStatus.BAD_GATEWAY);
    }
