...
public class StorageConfiguration {
    ...
    StorageService storageService(AppProperties properties) {
        ...
        String provider = storage.getProvider() == null ? "local" : storage.getProvider().trim().toLowerCase();
        ...
        S3Configuration s3Configuration = S3Configuration.builder().pathStyleAccessEnabled(provider.equals("minio")).build();
        S3ClientBuilder clientBuilder = S3Client.builder().credentialsProvider(credentials)
            .region(Region.of(storage.getRegion())).serviceConfiguration(s3Configuration);
        S3Presigner.Builder presignerBuilder = S3Presigner.builder().credentialsProvider(credentials)
            .region(Region.of(storage.getRegion())).serviceConfiguration(s3Configuration);
