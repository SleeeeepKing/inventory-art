...
浏览器通过已认证的后端接口上传待确认对象，不依赖 Bucket CORS。后端上传不会创建第二个文件，后续仍需通过大小、MIME 和 SHA-256 校验后才会绑定商品。

Cloudflare R2 的 Java SDK 客户端必须使用路径式访问、关闭 chunked encoding，并仅在协议要求时启用 SDK checksum，否则后端 `PutObject` 可能因签名或校验和不兼容而失败。如另行实现浏览器直传，仍应按下方策略配置 CORS。
