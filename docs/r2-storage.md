...
浏览器优先使用预签名 URL 直传对象存储。如果对象存储因 CORS 或边缘网络中断而完全没有返回 HTTP 响应，前端会把同一个待确认对象通过已认证的后端接口上传一次；该兜底不会创建第二个文件，后续仍需通过大小、MIME 和 SHA-256 校验后才会绑定商品。

Cloudflare R2 的 Java SDK 客户端必须使用路径式访问并关闭 chunked encoding，否则后端 `PutObject` 会因签名不匹配而失败。后端兜底不依赖 Bucket CORS；如需浏览器直传，仍应按下方策略配置 CORS。
