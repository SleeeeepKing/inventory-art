...
上传、分析、预览都不会修改订单、交易或库存。用户必须在预览影响后明确确认，后端才执行正式导入。

## 支持的数据粒度

导入向导自动识别，无法可靠判断时要求用户选择：

| 类型 | 每行含义 | 订单/交易 | 默认库存影响 | 主销售报表 |
| --- | --- | --- | --- | --- |
| `TRANSACTION_HISTORY` | 一笔支付、退款、手续费等交易 | 创建/更新 `external_transactions`；成功支付可建未分配订单 | 无商品明细时不改库存 | 逐笔交易或其关联订单只计一次 |
| `ORDER_HISTORY` | Ticket/Order 的一个商品行 | 按外部订单号聚合订单与明细 | 所有商品映射且库存足够、用户确认后扣减 | 计算聚合后的订单一次 |
| `PRODUCT_SALES` | 某周期商品销售汇总 | 写 `imported_sales_summaries`，不伪造顾客订单 | 默认不改；二次确认才生成流水 | 默认只用于对账，不与逐笔销售相加 |
| `ACCOUNTING_REPORT` | 日期/支付方式/税率汇总 | 写 `imported_accounting_summaries` | 无 | 只用于对账，不进入主销售额 |
| `UNKNOWN` | 不能可靠识别 | 不允许确认 | 无 | 无 |

交易级文件没有商品明细时，系统可以创建 `source=SUMUP_IMPORT`、`allocationStatus=UNALLOCATED` 的订单。它可以进入财务销售额，但不能进入具体商品销量，也不能扣减任何商品库存。
...
只有 `READY_FOR_CONFIRMATION` 才接受确认。确认请求携带 `analysisVersion`、最终商品映射、是否记住映射，以及商品汇总是否明确应用库存等用户决策。
...
- 分析和重映射按 `IMPORT_BATCH_SIZE=200` 分批写入；正式确认在一个数据库事务中完成，以保证订单、交易、库存和行状态原子一致。为避免长事务和内存失控，单批次硬性限制为 20,000 行。
- 行状态、业务记录、库存流水和批次进度在对应事务中一起提交。
- 系统级中断会回滚整个正式确认事务；用户可以安全重试整批确认，唯一约束和幂等引用防止重复订单、销售额或库存流水。当前版本不在单个批次内部断点续跑。
...
- 只有全部需要的商品映射成功且库存校验通过时，才能在确认事务中应用库存。
- 任一商品库存不足时，默认整张订单不应用库存；用户可以选择把整单作为未分配导入，不能产生部分扣减。
- 重复确认由订单 `inventory_applied`、导入行状态和唯一约束共同阻止。

### 商品销售汇总

- 保存为 `imported_sales_summaries`，默认只用于销售/库存对账，不伪造订单。
- 默认不改库存。用户选择“根据汇总销量调整库存”时，预览必须列出每个商品影响并进行第二次确认。
- 调整通过 `InventoryService` 创建 `SUMUP_IMPORT` 流水并关联批次；同一批次只能应用一次。

### 退款与状态更新

- 有订单项和退款数量时，只恢复尚未退款的对应数量，并记录退款明细。
- 只有退款金额、没有商品数量时，只更新财务状态/退款金额，不猜测库存。
- 重复退款行按 provider ID/fingerprint 和累计退款状态幂等处理，不能重复恢复库存。
...
4. 对已应用库存生成反向流水，不删除原流水、不直接重写历史。
5. 只失效由该批次创建且之后未被修改的订单/外部交易；ImportBatch 和 ImportRow 永久保留。
...
- 导入确认时的库存竞争按实际提交时重新检查，不能只相信预览时库存。
...
`backend/src/test/resources/sumup/` 应覆盖：UTF-8、BOM、Windows-1252、逗号/分号/Tab、英法表头、点/逗号小数、XLS/XLSX、空行、合计行、缺字段、未知格式、超限文件、重复 checksum、重复交易 ID、fingerprint、状态更新、全额/部分退款、自动/手动/失败商品映射、有/无商品库存影响、撤销和跨 Tenant 相同交易号。

## 未来 SumUp API 扩展点

文件导入和未来同步都应产生相同的规范外部交易模型，并复用去重、映射、订单、库存和报表策略。边界接口可以表达为 `ExternalSalesProvider`，当前只有文件 provider；未来 API provider 负责拉取/分页/游标和签名验证，但不能绕过：

- Tenant 认证和 provider credential 隔离；
- 相同 provider ID/fingerprint 规则；
- 预览或明确配置的自动确认策略；
- `InventoryService` 与订单幂等；
- 敏感字段清理和 AuditLog。
